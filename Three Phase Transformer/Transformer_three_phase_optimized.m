...
Vp_star_line = 10*10^3;    % Primary voltage line star conf. [V]

Vs = 400;                  % Secondary voltage [V]
Vs_tri_line = Vs*sqrt(3);  % Secondary voltage line triangle conf. [V]
...
for B_core = B_core_range
    for J_winding = J_winding_range
        for K_insulation = K_insulation_range
            ...
            ks = Vp_star_line/Vs_tri_line;    % Trasformation rate

            % Calculate primary and secondary current [A]
            Ip = Sn/(sqrt(3) * Vp_star_line);
            Is = Sn/(sqrt(3) * Vs_tri_line);

            % Number of primary and secondary coil
            N1 = ceil(Vp_star_line/(sqrt(3)*K_insulation));
            N2 = ceil(N1 * Vs/Vp);
            ...
            for h_windings = h_windings_range
                ...
                R0 = Vp_star_line^2 / (Ploss_steel/3);

                % Magnetizing Reactance X0 [Ohm]
                X0 = 2 * pi * f * (N1 + N2)^2 * (mu_steel * A_core / lenght_magnetic);
